...
def relocate(grid, R, threshold, open_loc):
    '''
    ...
    for i in ns:
        if is_satisfied(grid, R, threshold, i):
            continue
        else:
            for j in open_loc:
...
def do_simulation(grid, R, threshold, max_steps):
    ...
    for i in range(max_steps):
        ns1 = not_satisfied(grid, R, threshold)
        if len(not_satisfied(grid, R, threshold)) == 0:
            counts = counts + 1
            break
        counts = counts + 1
        relocate(grid, R, threshold, open_loc)
        ns2 = not_satisfied(grid, R, threshold)
        if ns1 == ns2:
            break
    return counts
